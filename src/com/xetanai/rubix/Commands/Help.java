	...
	public void onCalled(Bot bot, MessageReceivedEvent msg)
	{
		...
		if (params.length == 1)
		{
			...
			for (Command cmd : bot.getCommandList())
				post+=bot.getSettings().getPrefix() + cmd.getHelp(false)+"\n";
			post += "```\nUse "+ bot.getSettings().getPrefix() +"help <command> to get indepth information on a command.";
			sendMessage(bot, msg, post);
			return;
		}
		for (Command cmd : bot.getCommandList())
			if (cmd.getKeyword().equals(params[1]))
				sendMessage(bot, msg, "```Usage: !"+ cmd.getHelp(true) +"```");
